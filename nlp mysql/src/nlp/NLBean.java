...
public class NLBean extends Panel {
    ...
    private String[] databaseNames = {"jdbc:mysql://localhost:3306/nlp", "jdbc:oracle:thin:@localhost:1521:xe"};
    private String[] userNames = {"root", "nlp"};
    private String[] passwords = {"admin", "nlp"};
    // set up a list of table names for each database:
    private String[] tableLists = {"students;emp", ""};
    ...
    public NLBean() {
        ...
        help_frame.setBackground(Color.white);
        ...
        Label l1 = new Label("Human Language Query Processing - A6");
        ...
        list1 = new java.awt.List(3, false);
            ...
            list1.add("Welcome to HLQP");
            
        
        
        list1.add("Use the help options to understand clearly");
            list1.add("A project initiavtive by Team 6");

        add(list1);
        
        list1.setBounds(100, 40, 400, 90);

        Button q_button = new Button("Do query");
        q_button.addMouseListener(new MouseQuery());
        add(q_button);
        q_button.setBounds(2, 140, 160, 30);
        Button help_button = new Button("Help");
        help_button.addMouseListener(new MouseHelp());
        add(help_button);
        help_button.setBounds(172, 140, 40, 30);

        Label label22 = new Label("Query:");
        label22.setFont(new Font("Dialog", Font.BOLD, 14));
        add(label22);
        label22.setBounds(2, 180, 60, 22);
        label22.setVisible(true);

        //        inputText = new SmartTextField("list Salary where EmpName equals Mark", 64);
        inputText = new TextField("Type your query here", 64);
        add(inputText);
        inputText.setBounds(80, 180, 500, 22);
        ...
        add(choice);
        choice.setBounds(2, 210, 582, 25);

        Label label23 = new Label("Generated SQL:");
        label23.setFont(new Font("Dialog", Font.BOLD, 12));
        add(label23);
        label23.setBounds(2, 240, 120, 30);
        sqlText = new TextArea("", 1, 80, TextArea.SCROLLBARS_NONE);
        sqlText.setEditable(true);
        add(sqlText);
        sqlText.setBounds(130, 240, 455, 40);

        outputText = new TextArea("Human Language Query Processing", 8, 74);
        add(outputText);
        outputText.setBounds(2, 285, 582, 150);

        Label l1x = new Label("Human Language Query Processing A6");
        l1x.setFont(new Font("Dialog", Font.BOLD, 16));
        add(l1x);
        l1x.setBounds(5, 442, 537, 19);

        list1_last_selection = -1;
        list2_last_selection = -1;
        list3_last_selection = -1;
     ...
     * field; otherwise, set the input text field (from one of the example
     * strings) and return a zero length string.
     */
    ...
    /**
     * utility method for getting the input text (either that the user typed in
     * or set from one of the example strings), calling the NLP engine's
    ...
    public static void main(String args[]) {
        ...
        f.add(sc);
        f.setTitle("Human Language Query Processing Beta Version");
        f.pack();
        f.show();
    ...
    /**
     * data for specifying a few sample natural language queries to place at the
     * beginning of the 'Example' 'choice' control.
     */
    private String[] examples = {
        "list deptno where dname equals accounting",
        "list loc",
        "list grade where losal equals 700 and hisal equals 1200",
        "display sal",
        "show dname where deptno equals 10",
        "show mgr where hiredate equals 1988-11-17 00:00:00",
        "list ename where sal is less than 2000"
    };
